-- ==============================================
-- BALANCE MONITORING REAL-TIME FIXES (SIMPLE VERSION)
-- ==============================================
...
-- 3. Add column to partners table for shared credentials (if not exists)
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'partners' AND column_name = 'use_shared_credentials') THEN
        ALTER TABLE partners ADD COLUMN use_shared_credentials BOOLEAN DEFAULT true;
    END IF;
END $$;
...
-- 6. Add indexes for performance (if not exist)
...
-- 7. Insert default shared credentials (only if not exists)
