-- SMS Database Tables Creation Script (Simple Version)
...
-- 4. Create sms_notifications table (without foreign key constraint for now)
...
-- 7. Insert default SMS templates for existing partners (only if they don't exist)
...
AND NOT EXISTS (
    ...
    WHERE st.partner_id = p.id AND st.template_name = 'Welcome Message'
);
...
AND NOT EXISTS (
    SELECT 1 FROM sms_templates st 
    WHERE st.partner_id = p.id AND st.template_name = 'Loan Disbursement'
);
...
AND NOT EXISTS (
    SELECT 1 FROM sms_templates st 
    WHERE st.partner_id = p.id AND st.template_name = 'Payment Confirmation'
);
