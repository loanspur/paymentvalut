-- ==============================================
-- BALANCE MONITORING FIXES - ROOT CAUSE RESOLUTION
-- ==============================================
-- This migration fixes the stale balance data issues by:
-- 1. Implementing single source of truth
-- 2. Adding shared credentials support
-- 3. Fixing timing logic
-- 4. Improving callback handling

-- ==============================================
-- 1. CREATE SHARED CREDENTIALS TABLE
-- ==============================================
...
-- ==============================================
-- 2. FIX BALANCE MONITORING CONFIG
-- ==============================================
-- Ensure balance_monitoring_config has proper defaults for real-time checking
UPDATE balance_monitoring_config 
SET 
    check_interval_minutes = 1,  -- Check every minute for real-time updates
    is_enabled = true
WHERE check_interval_minutes > 5;  -- Only update if currently set to slow intervals

-- ==============================================
-- 3. CREATE REAL-TIME BALANCE TRIGGER FUNCTION
-- ==============================================
CREATE OR REPLACE FUNCTION trigger_realtime_balance_check()
RETURNS TRIGGER AS $$
BEGIN
    -- Trigger balance check when balance_requests table is updated
...
-- ==============================================
-- 4. CREATE BALANCE CHECK LOGS TABLE
-- ==============================================
CREATE TABLE IF NOT EXISTS balance_check_logs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    partner_id UUID NOT NULL REFERENCES partners(id) ON DELETE CASCADE,
    action VARCHAR(50) NOT NULL, -- 'balance_updated', 'check_triggered', 'callback_received'
...
-- ==============================================
-- 5. CREATE TRIGGER FOR REAL-TIME UPDATES
-- ==============================================
DROP TRIGGER IF EXISTS balance_requests_realtime_trigger ON balance_requests;
CREATE TRIGGER balance_requests_realtime_trigger
    AFTER UPDATE ON balance_requests
    FOR EACH ROW
    EXECUTE FUNCTION trigger_realtime_balance_check();
...
-- ==============================================
-- 7. INSERT DEFAULT SHARED CREDENTIALS
-- ==============================================
-- Insert default shared credentials (these should be updated with real values)
...
-- ==============================================
-- 8. UPDATE EXISTING PARTNERS TO USE SHARED CREDENTIALS
-- ==============================================
-- Add a flag to indicate if partner should use shared credentials
ALTER TABLE partners ADD COLUMN IF NOT EXISTS use_shared_credentials BOOLEAN DEFAULT true;

-- ==============================================
-- 9. CREATE REAL-TIME BALANCE API FUNCTION
-- ==============================================
...
BEGIN
    ...
    SELECT 
        ...
        br.updated_at,
        CASE 
            WHEN br.updated_at > NOW() - INTERVAL '1 hour' THEN 'fresh'
            WHEN br.updated_at > NOW() - INTERVAL '24 hours' THEN 'recent'
            ELSE 'stale'
...
-- ==============================================
-- 10. CREATE FORCE BALANCE CHECK FUNCTION
-- ==============================================
...
BEGIN
    ...
    UPDATE balance_monitoring_config 
    SET last_checked_at = NOW() - INTERVAL '1 hour'  -- Force check by setting old timestamp
...
-- ==============================================
COMMENT ON TABLE shared_mpesa_credentials IS 'Shared M-Pesa credentials for all partners to use single source of truth';
COMMENT ON TABLE balance_check_logs IS 'Logs all balance check activities for debugging and monitoring';
COMMENT ON FUNCTION get_realtime_balance(UUID) IS 'Gets the most recent balance data for a partner with freshness indicator';
COMMENT ON FUNCTION force_balance_check(UUID) IS 'Forces an immediate balance check for a specific partner';

-- ==============================================
-- 12. CREATE VIEW FOR REAL-TIME BALANCE DASHBOARD
-- ==============================================
...
COMMENT ON VIEW realtime_balance_dashboard IS 'Real-time balance dashboard showing current balances and freshness for all partners';

-- ==============================================
-- MIGRATION COMPLETE
-- ==============================================
-- This migration addresses all root causes:
-- ✅ Single source of truth (balance_requests table only)
-- ✅ Shared credentials support
-- ✅ Real-time checking (1-minute intervals)
-- ✅ Proper callback handling (35-second wait)
