-- Robust Audit Trail Migration
-- This migration checks for column existence before creating functions
...
-- 4. Create functions with column existence checks

-- Function for audit_logs table (check if columns exist first)
DO $$
BEGIN
    ...
    IF EXISTS (
        ...
        AND column_name IN ('action', 'resource_type', 'resource_id', 'old_values', 'new_values', 'user_id', 'ip_address', 'user_agent')
    ) THEN
        -- Create the function for audit_logs
        ...
        RAISE NOTICE 'Created log_audit_event function for audit_logs table';
    ELSE
        RAISE NOTICE 'Skipping log_audit_event function - required columns not found in audit_logs table';
    END IF;
END $$;

-- Function for user_activities table (check if columns exist first)
DO $$
BEGIN
    -- Check if user_activities table has the required columns
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'user_activities' 
        AND table_schema = 'public'
        AND column_name IN ('user_id', 'action', 'ip_address', 'user_agent')
    ) THEN
        -- Create the function for user_activities (without description column)
        ...
        BEGIN
            INSERT INTO user_activities (
                ...
                p_user_id, p_action, p_ip_address, p_user_agent
            );
        END;
        $$ LANGUAGE plpgsql;
        
        RAISE NOTICE 'Created log_user_activity function for user_activities table';
    ELSE
        RAISE NOTICE 'Skipping log_user_activity function - required columns not found in user_activities table';
    END IF;
END $$;
...
BEGIN
    INSERT INTO api_requests (
        ...
        p_response_status, response_headers, p_response_body, p_response_time_ms,
...
-- 6. Test the functions with sample data (only if tables exist)
DO $$
BEGIN
    ...
    -- Test log_user_activity function if user_activities table has required columns
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'user_activities' 
        AND table_schema = 'public'
        AND column_name IN ('user_id', 'action', 'ip_address')
    ) THEN
        ...
        RAISE NOTICE 'Tested log_user_activity function';
    END IF;
