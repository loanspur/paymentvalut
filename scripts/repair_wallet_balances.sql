-- WARNING: This script attempts to repair wallet balance inconsistencies
-- Review the diagnostic queries first before running this repair script
-- BACKUP YOUR DATABASE BEFORE RUNNING THIS SCRIPT
-- 
-- IMPORTANT: Run scripts/diagnose_wallet_balance_issues.sql FIRST to understand the scope
--
-- This script creates missing wallet_transactions records based on:
-- 1. C2B transactions that have partner_id but no wallet_transactions (creates top_up transactions)
-- 2. Wallets with balance but no transactions (creates initial balance transaction as manual_credit)
--
-- The repair creates transactions that represent the "missing" balance changes.
-- After running this, balances should be consistent, but the transaction history may not
-- perfectly reflect the original transaction flow (since we're reconstructing from balances).
...
-- (e.g., wallet updated after last transaction was created)
...
-- This represents the "opening balance" that existed before proper tracking
INSERT INTO wallet_transactions (
    wallet_id,
    transaction_type,
    amount,
...
WHERE pw.current_balance != 0
  ...
  -- Only create if we haven't already created a transaction for this wallet in STEP 1
  AND NOT EXISTS (
      SELECT 1
      FROM wallet_transactions wt2
