...
serve(async (req) => {
  ...
  try {
    ...
    if (!balanceError && balanceRequest) {
      ...
      console.log('🔍 [M-Pesa Callback] ResultParameters:', Result.ResultParameters)

      if (Result.ResultParameters?.ResultParameter) {
        console.log('🔍 [M-Pesa Callback] Processing ResultParameter array:', Result.ResultParameters.ResultParameter)
        ...
        for (const param of Result.ResultParameters.ResultParameter) {
          ...
          switch (param.Key) {
            case 'AccountBalance':
              ...
              console.log(`✅ [M-Pesa Callback] Found AccountBalance: ${balanceAfter}`)
              break
            case 'B2CUtilityAccountAvailableFunds':
              utilityAccountBalance = parseFloat(param.Value)
              console.log(`✅ [M-Pesa Callback] Found B2CUtilityAccountAvailableFunds: ${utilityAccountBalance}`)
              break
            case 'B2CWorkingAccountAvailableFunds':
              balanceBefore = parseFloat(param.Value)
              console.log(`✅ [M-Pesa Callback] Found B2CWorkingAccountAvailableFunds: ${balanceBefore}`)
              break
            case 'WorkingAccountAvailableFunds':
              balanceBefore = parseFloat(param.Value)
              console.log(`✅ [M-Pesa Callback] Found WorkingAccountAvailableFunds: ${balanceBefore}`)
              break
            case 'UtilityAccountAvailableFunds':
              utilityAccountBalance = parseFloat(param.Value)
              console.log(`✅ [M-Pesa Callback] Found UtilityAccountAvailableFunds: ${utilityAccountBalance}`)
              break
            case 'AvailableBalance':
              balanceAfter = parseFloat(param.Value)
              console.log(`✅ [M-Pesa Callback] Found AvailableBalance: ${balanceAfter}`)
              break
            case 'Balance':
              balanceAfter = parseFloat(param.Value)
              console.log(`✅ [M-Pesa Callback] Found Balance: ${balanceAfter}`)
              break
            default:
              console.log(`ℹ️ [M-Pesa Callback] Unknown parameter: ${param.Key} = ${param.Value}`)
              break
          }
        }
      } else {
        console.log('❌ [M-Pesa Callback] No ResultParameters found in response')
      }
      ...
      console.log('📊 [M-Pesa Callback] Balance data extracted:', {
