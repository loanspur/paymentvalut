...
async function testOTPSMSExactFlow() {
  try {
    console.log('🧪 Testing Exact OTP SMS Flow...');
    console.log('==================================');
    ...
    // Step 2: Get SMS settings (same logic as OTP system)
    ...
    // Step 3: Create OTP record (same as OTP system)
    ...
    // Step 4: Send SMS (exact same logic as OTP system)
    ...
    // Determine if the SMS settings are encrypted (same logic as OTP system)
    const isEncrypted = !(user.role === 'super_admin' && process.env.SUPER_ADMIN_SMS_ENABLED === 'true');
    console.log('Is Encrypted:', isEncrypted);
    
    // Send SMS via AirTouch API (exact same function as OTP system)
    ...
    if (smsResponse.success) {
      ...
      // Update OTP record (same as OTP system)
      await supabase
        .from('login_otp_validations')
